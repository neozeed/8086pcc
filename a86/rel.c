...
//printf("Rel_Header Rel_name (tout) is %s\n",Rel_name);
...
//printf("Rel_Header Source_name (rtout) is %s\n",rname);
	rtout = fopen(rname, "wb");
	Concat(rname, Source_name, ".datar");
//printf("Rel_Header Source_name (rdout) is %s\n",rname);
	rdout = fopen(rname, "wb");
	if (rtout == NULL || rdout == NULL)
	  Sys_Error("open on output file %s failed", rname);

//printf("ftell on tout %d\n",ftell(tout));
	fseek(tout, (long)N_TXTOFF(filhdr), SEEK_SET);	/* seek to start of text */
//printf("ftell on tout %d\n",ftell(tout));
//printf("ftell on dout %d\n",ftell(dout));
	fseek(dout, (long)N_TXTOFF(filhdr)+tsize, SEEK_SET);	
//printf("ftell on dout %d\n",ftell(dout));
	rtsize = 0;
	rdsize = 0;
}
...
//printf("ftell on dout %d\n",ftell(dout));
	fseek(dout, (long)(N_TXTOFF(filhdr)+tsize+dsize), SEEK_SET);
//printf("ftell on dout %d\n",ftell(dout));
