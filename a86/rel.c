...
#else
        fseek(tout, (long)      ((filhdr).a_magic==0413          ? 1024 : sizeof (struct exec)), 0);
        fseek(dout, (long)      ((filhdr).a_magic==0413          ? 1024 : sizeof (struct exec))+tsize, 0);
#endif
	rtsize = 0;
	rdsize = 0;
}
...
#else
        fseek(dout, (long)(     ((filhdr).a_magic==0413          ? 1024 : sizeof (struct exec))+tsize+dsize), 0);
#endif
